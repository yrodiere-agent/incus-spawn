...
assert_eq() {
    ...
    if [ "$actual" = "$expected" ]; then
        ...
        printf '  \033[31mFAIL\033[0m  %s  (actual: %s, expected: %s)\n' \
...
find_template_pool() {
    find_instance_pool "tpl-minimal"
}
...
BRANCH_POOL=$(find_instance_pool "cow-test-branch")
echo "  Branch pool: $BRANCH_POOL"
echo ""
...
echo "--- 6. Assertions ---"
assert_eq "Branch landed on same pool as template" "$BRANCH_POOL" "$POOL"
