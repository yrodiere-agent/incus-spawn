...
**Upstream delegates:** When `proxy.delegates` is configured in `config.yaml`, the proxy routes outbound traffic for specific domains through an intermediate HTTP/HTTPS proxy instead of connecting directly to the real upstream. The MITM proxy still terminates TLS from the container, injects credentials, and then forwards the complete request (with real auth headers) to the delegate. This enables integration with cost-reduction proxies (e.g. headroom for LLM API context compression) or corporate reverse proxies. The delegate lookup is by exact domain match and uses the resolved upstream host — so Vertex-translated requests to `aiplatform.googleapis.com` do not match a delegate configured for `api.anthropic.com`.
